		...
		if scraped_image_url:
			...
			for i, form in enumerate(product_formset.forms):
				...
				if not files_dict.get(product_banner_field):
					scraped_product_image = download_image_from_url(
						scraped_image_url, f"product_{i}_banner"
					)
		...
		if url_extractor_form.is_valid():
			...
			if not result.get("success"):
				url_extractor_form.add_error(
					"url", result.get("error", "擷取失敗，請檢查網址")
				)
				return render(request, "groups/new.html", context)

			first_image = result.get("main_image") or (
			...
			if first_image and group_description:
				group_description = (
					f'<img src="{first_image}" alt="商品圖片"><br>{group_description}'
				)
			elif first_image:
			...
			valid_variants = {
				k: v
				for k, v in result.get("variants", {}).items()
			...
			if valid_variants:
				...
				DynamicProductFormSet = formset_factory(
					ProductForm, extra=combo_count, max_num=combo_count
				)
				...
				for i, combo in enumerate(combinations):
					if i < len(product_formsets.forms):
						...
						form.initial["description"] = (
							product_description or result.get("name", "") or ""
						)
			else:
...
def download_image_from_url(image_url, filename_prefix="scraped_image"):
	try:
		...
		response = requests.get(image_url, timeout=10, headers=headers, verify=verify_ssl)
